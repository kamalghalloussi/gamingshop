...
if(isset($_FILES['image'])){
    ...
    $photo_produit = $repertoireDestination . basename($_FILES['image']['name']);
   
    $_POST['image'] = $photo_produit;
    if(move_uploaded_file($_FILES['image']['tmp_name'], $photo_produit)){
...
if($dbh){
    $sql = "INSERT INTO `produits`(`id_produit`, `nom_du_produit`, `description`, `prix_produit`, `stock_produit`, `date_dépot`, `image`) VALUES (?,?,?,?,?,?,?)";

    $insert = $dbh->prepare($sql);

    $insert->bindParam(1, $_POST['id_produit']);
    $insert->bindParam(2, $_POST['nom_du_produit']);
    ...
    $insert->bindParam(7, $_POST['image']);


    $insert->execute(array(
        $_POST['id_produit'],
        $_POST['nom_du_produit'],
        $_POST['description'],
        $_POST['prix_produit'],
        $_POST['stock_produit'],
        $_POST['date_dépot'],
